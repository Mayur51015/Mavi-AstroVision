...
import { getSunSign, getMoonSign, getAscendant } from '../services/astrologyApiService.js';
...
export const addBirthDetails = async (req, res) => {
  try {
    ...
    // Try to fetch astrological signs from API
    try {
      const sunSignData = await getSunSign(birthDetail);
      const moonSignData = await getMoonSign(birthDetail);
      const ascendantData = await getAscendant(birthDetail);

      if (sunSignData) birthDetail.sunSign = sunSignData.sign;
      if (moonSignData) birthDetail.moonSign = moonSignData.sign;
      if (ascendantData) birthDetail.ascendant = ascendantData.sign;
    } catch (apiErr) {
      console.log('API enrichment skipped:', apiErr.message);
    }
